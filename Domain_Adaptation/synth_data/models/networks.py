...
def define_D(input_nc, ndf, netD, fc_input_nc=512*4*4, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = LeNet(input_nc, fc_input_nc, output_dim=2)
    else:
...
#     def __init__(self, input_nc, fc_input_nc, output_dim):
#         super().__init__()

#         self.net = nn.Sequential(GradientReversal(),
#         nn.Conv2d(in_channels = input_nc, out_channels = 256, kernel_size = 3),
#         nn.ReLU(),
#         nn.BatchNorm2d(256),
#         nn.Dropout(0.5),
...
#         self.pred = nn.Sequential(nn.Linear(fc_input_nc, 128),
#         nn.BatchNorm1d(128),
#         nn.Dropout(0.5),
#         nn.ReLU(),
#         nn.Linear(128, 32),
...
class LeNet(nn.Module):
    ...
    def __init__(self, input_nc, fc_input_nc, output_dim):
        ...
        self.pred = nn.Sequential(nn.Linear(fc_input_nc, 32),
        nn.BatchNorm1d(32),
        nn.Dropout(0.5),
        nn.ReLU(),
        nn.Linear(32, output_dim))
