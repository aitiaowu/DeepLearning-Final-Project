...
class ourmodel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_DA', type=float, default=100.0, help='weight for DA loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_Entropy']
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(self.netD1.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def set_input_train(self, input, target_input):
        ...
        self.input_x = torch.cat([self.real_A, self.target_real_A])
        self.discriminator_y = torch.cat([torch.zeros(self.real_A.shape[0], device=self.device).long(), torch.ones(self.target_real_A.shape[0], device=self.device).long()])
        # self.discriminator_y = torch.cat([torch.zeros(self.real_A.shape[0], device=self.device).float(), torch.ones(self.target_real_A.shape[0], device=self.device).float()])
    ...
    def test(self):
        ...
        with torch.no_grad():
            self.forward()
            pred_depth_style = self.netD4(self.source_features[3])
            ...
            self.val_loss_names = ['IoU_background',	'IoU_1', 'IoU_2', 'IoU_3', 'IoU_4', 'IoU_5', 'IoU_mean', 'style_correct_num']
    ...
    def validate(self, val_dataset, name):
